...
func (s *transferCommandService) CreateTransaction(ctx context.Context, request *requests.CreateTransferRequest) (*models.Transfer, error) {
	...
	if request.IdempotencyKey != "" && s.idempotencyStore != nil {
		...
		if _, err := s.idempotencyStore.Claim(ctx, "transfer", request.IdempotencyKey, idemHash); err == nil {
			...
			if gErr != nil {
				status = "error"
				return errorhandler.HandleError[*models.Transfer](s.logger, gErr, method, span, zap.String("idempotency_key", request.IdempotencyKey))
			}
			if existing.RequestHash != idemHash {
				status = "error"
				return errorhandler.HandleError[*models.Transfer](s.logger, idempotency.ConflictError(), method, span, zap.String("idempotency_key", request.IdempotencyKey))
			}
			if existing.Status == idempotency.StatusSuccess {
				...
				if uErr := json.Unmarshal(existing.ResponseJSON, &row); uErr != nil {
					status = "error"
					return errorhandler.HandleError[*models.Transfer](s.logger, uErr, method, span, zap.String("idempotency_key", request.IdempotencyKey))
				}
				logSuccess("Replayed idempotent transfer response", zap.String("idempotency_key", request.IdempotencyKey))
				return &row, nil
			}
			status = "error"
			return errorhandler.HandleError[*models.Transfer](s.logger, idempotency.ProcessingError(), method, span, zap.String("idempotency_key", request.IdempotencyKey))
		}
	}

	if err := validation.ValidateAmount(request.TransferAmount); err != nil {
		status = "error"
		return errorhandler.HandleError[*models.Transfer](s.logger, err, method, span, zap.String("from_card", security.MaskCardNumber(request.TransferFrom)))
	}
	if err := validation.ValidateTransferParties(request.TransferFrom, request.TransferTo); err != nil {
		status = "error"
		return errorhandler.HandleError[*models.Transfer](s.logger, err, method, span, zap.String("from_card", security.MaskCardNumber(request.TransferFrom)), zap.String("to_card", security.MaskCardNumber(request.TransferTo)))
	}
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.Transfer](s.logger, err, method, span, zap.String("to_card", security.MaskCardNumber(request.TransferTo)))
	}
	...
	if int(senderSaldo.TotalBalance) < request.TransferAmount {
		...
		return errorhandler.HandleError[*models.Transfer](s.logger, err, method, span, zap.String("from_card", security.MaskCardNumber(request.TransferFrom)), zap.Float64("balance", float64(senderSaldo.TotalBalance)), zap.Float64("amount", float64(request.TransferAmount)))
	}
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.Transfer](s.logger, err, method, span)
	}
	...
	if ok, gErr := s.transferCommandRepository.GuardStatus(ctx, transferID, state.Pending, state.Processing, ""); gErr != nil || !ok {
		...
		return errorhandler.HandleError[*models.Transfer](s.logger, gErr, method, span, zap.Int("transfer_id", transferID))
	}
	...
	if err != nil {
		...
		status = "error"
		return errorhandler.HandleError[*models.Transfer](s.logger, err, method, span, zap.String("failed_to_credit", "receiver"))
	}
	...
	if err != nil {
		...
		return errorhandler.HandleError[*models.Transfer](s.logger, err, method, span, zap.Int("transfer_id", transferID))
	}
...
func (s *transferCommandService) UpdateTransaction(ctx context.Context, request *requests.UpdateTransferRequest) (*models.Transfer, error) {
	...
	if err != nil {
		...
		return errorhandler.HandleError[*models.Transfer](s.logger, err, method, span, zap.Int("transfer_id", *request.TransferID))
	}
	...
	if err != nil {
		status = "error"
		s.markTransferAsFailed(ctx, *request.TransferID, method, span)
		return errorhandler.HandleError[*models.Transfer](s.logger, err, method, span, zap.String("from_card", security.MaskCardNumber(transfer.TransferFrom)))
	}
	...
	if err != nil {
		status = "error"
		s.markTransferAsFailed(ctx, *request.TransferID, method, span)
		return errorhandler.HandleError[*models.Transfer](s.logger, err, method, span, zap.String("to_card", security.MaskCardNumber(transfer.TransferTo)))
	}
	...
	if amountDifference > 0 {
		...
		if _, err = s.saldoAdapter.DebitSaldo(ctx, &requests.DebitSaldoRequest{
			...
			s.markTransferAsFailed(ctx, *request.TransferID, method, span)
			return errorhandler.HandleError[*models.Transfer](s.logger, err, method, span, zap.String("failed_to_debit", "receiver"))
		}
	}
...
func (s *transferCommandService) enqueueTransferEvents(ctx context.Context, transfer *models.Transfer, updatedTransfer *models.Transfer, senderCard *models.Card, request *requests.CreateTransferRequest, senderNewBalance, receiverNewBalance int) {
	...
	if err == nil {
		...
		if iErr := s.outboxStore.Insert(ctx, repository.OutboxRecord{AggregateType: "transfer", AggregateID: tID, EventType: "transfer.created", Payload: emailPayload}); iErr != nil {
	...
	if iErr := s.outboxStore.Insert(ctx, repository.OutboxRecord{AggregateType: "transfer", AggregateID: request.TransferFrom, EventType: "transfer.saldo_sender", Payload: senderSaldoBytes}); iErr != nil {
	...
	if iErr := s.outboxStore.Insert(ctx, repository.OutboxRecord{AggregateType: "transfer", AggregateID: request.TransferTo, EventType: "transfer.saldo_receiver", Payload: receiverSaldoBytes}); iErr != nil {
