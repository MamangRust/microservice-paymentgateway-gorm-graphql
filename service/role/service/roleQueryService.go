...
func (s *roleQueryService) FindAll(ctx context.Context, req *requests.FindAllRoles) ([]*models.Role, *int, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandlerErrorPagination[[]*models.Role](
			s.logger,
	...
	totalCount64, err := s.roleQuery.CountAllRoles(ctx, search)
	if err != nil {
		status = "error"
		return errorhandler.HandlerErrorPagination[[]*models.Role](
			s.logger,
			sharedErrors.ErrFailed("count roles"),
			method,
			span,
		)
	}
...
func (s *roleQueryService) FindByActiveRole(ctx context.Context, req *requests.FindAllRoles) ([]*models.Role, *int, error) {
	...
	totalCount64, err := s.roleQuery.CountActiveRoles(ctx, search)
	if err != nil {
		status = "error"
		return errorhandler.HandlerErrorPagination[[]*models.Role](
			s.logger,
			sharedErrors.ErrFailed("count active roles"),
			method,
			span,
		)
	}
...
func (s *roleQueryService) FindByTrashedRole(ctx context.Context, req *requests.FindAllRoles) ([]*models.Role, *int, error) {
	...
	totalCount64, err := s.roleQuery.CountTrashedRoles(ctx, search)
	if err != nil {
		status = "error"
		return errorhandler.HandlerErrorPagination[[]*models.Role](
			s.logger,
			sharedErrors.ErrFailed("count trashed roles"),
			method,
			span,
		)
	}
